...
export default async function AwsPage({
  ...
  let data
  try {
    data = await getAwsMonthlyCosts(range.start, range.end, range.granularity ?? 'weekly')
  } catch (e) {
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        {data.stackedHistory && data.stackedHistory.length > 0
          ? <StackedServiceChart data={data.stackedHistory} title={`Consumo AWS por ${(range.granularity ?? 'weekly') === 'monthly' ? 'Mes' : 'Semana'}`} />
